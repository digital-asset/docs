...
A validator can connect to additional synchronizers beyond the Global Synchronizer. Dedicated Synchronizers are operated by enterprises or consortia for their own application domains. Contracts on different synchronizers can interact through reassignment (transfer) operations, which move a contract from one synchronizer to another.
