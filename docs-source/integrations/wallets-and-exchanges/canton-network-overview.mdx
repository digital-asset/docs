...
To use a party, you must **onboard** it by submitting a topology transaction that authorizes a node to host it. The designated node must then submit a matching transaction to officially accept the hosting request. See [Creating an External Party](/sdks-tools/sdks/wallet-sdk/guides/creating-an-external-party) for step-by-step instructions.
